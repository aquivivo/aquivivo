...
function orthographyHint(userRaw, expectedRaw) {
  ...
  if (expectedPlain.includes(userPlain) && !expectedList.includes(user)) {
    return 'Pista: falta algun signo polaco (Ä…, Ä‡, Ä™, Å‚, Å„, Ã³, Å›, Å¼, Åº).';
  }

  const expectedJoined = expectedList.join(' ');
  if (/(ch|cz|sz|rz|dÅ¼|dÅº)/i.test(expectedJoined)) {
    return 'Pista: revisa digrafos polacos (h/ch/cz/sz/rz/dÅ¼/dÅº).';
  }
...
function presentableTheoryNotes(raw, ex = null) {
  ...
  const hiddenPronunciation =
    /^(?:[-*]\s*)?(?:pron\.?|wymowa|pronunciacion|pronunciaciÃ³n)\s*:/i;
  ...
  const filtered = out.filter((line) => {
    ...
    if (/^(?:es|trad|traduccion|traducciÃ³n|instruccion|instrucciÃ³n)\s*:/i.test(text)) return false;
...
async function markCurrentAsDontKnow() {
  ...
  await appendMistakeLog(ex, '[no sÃ©]', expected);
  ...
  setProgressUI();
  showToast(
    expected ? `No pasa nada. Respuesta correcta: ${expected}` : 'Marcado como "No sÃ©".',
    'warn',
    2400,
  );
...
function showFinishModal(stats) {
  ...
  if (btnLesson) {
    btnLesson.textContent = stats.completed ? 'Volver a la lecciÃ³n' : 'Volver al mÃ³dulo';
  }
  if (btnCourse) {
