...
async function toggleRecording() {
  if (!btnRecord) return;
  if (!canRecord()) {
    setCommentMsg('Tu navegador no soporta grabaciÃ³n.', true);
    return;
  }
...
function renderPost(post, ownerUid) {
  ...
  if (subtitleEl) {
    subtitleEl.textContent = `${level ? `Nivel ${level}` : 'Comunidad'}${topicTitle ? ` Â· ${topicTitle}` : ''}`;
  }
...
function renderComments(list) {
  ...
  if (!items.length) {
    if (commentsHint) commentsHint.textContent = 'AÃºn no hay respuestas.';
    return;
  }
...
async function loadPostAndComments() {
  ...
  try {
    ...
    if (String(post.type || '') !== 'correction') {
      if (subtitleEl) subtitleEl.textContent = 'No es una correcciÃ³n.';
      if (postHost) postHost.innerHTML = '<div class="muted">Este post no es una solicitud de correcciÃ³n.</div>';
      if (btnSend) btnSend.disabled = true;
      return;
    }
...
async function sendComment() {
  ...
  if (recording) {
    setCommentMsg('DetÃ©n la grabaciÃ³n primero.', true);
    return;
  }
  ...
  if (text.length > 2200) {
    setCommentMsg('Texto demasiado largo (mÃ¡x. 2200 caracteres).', true);
    return;
  }
  ...
  try {
    ...
    if (voice) {
      ...
      if (voice.size > MAX_VOICE_BYTES) {
        setCommentMsg('Audio demasiado grande (mÃ¡x. 10MB).', true);
        return;
      }
