...
async function uploadAvatar(uid, emailLower, file, prevPath) {
  ...
  if (file.size > 10 * 1024 * 1024) {
    setInlineMsg(avatarMsg, 'La imagen es demasiado grande (mÃ¡x. 10MB).', true);
    return null;
  }
...
async function saveProfileBasics() {
  ...
  if (handleRaw && !isHandleValid(handleLower)) {
    setInlineMsg(profileMsg, 'Usuario invÃ¡lido. Usa 3â€“20 letras/nÃºmeros y . _ -', true);
    return;
  }
  ...
  try {
    ...
    setInlineMsg(profileMsg, 'Guardado âœ…');
...
async function savePreferences() {
  ...
  try {
    ...
    setInlineMsg(prefsMsg, 'Guardado âœ…');
...
async function saveBio() {
  ...
  try {
    ...
    setInlineMsg(bioMsg, 'Guardado âœ…');
...
async function savePrivacy() {
  ...
  try {
    ...
    setInlineMsg(privacyMsg, 'Guardado âœ…');
...
async function saveReviewPlan() {
  ...
  try {
    ...
    setInlineMsg(reviewSaveStatus, 'Guardado âœ…');
    ...
    avatarFile.addEventListener('change', async (e) => {
      ...
      try {
        ...
        if (res?.url) {
          ...
          setInlineMsg(avatarMsg, 'Foto guardada âœ…');
  ...
  if (btnAvatarRemove && !btnAvatarRemove.dataset.wired) {
    ...
    btnAvatarRemove.addEventListener('click', async () => {
      ...
      try {
        ...
        setInlineMsg(avatarMsg, 'Foto eliminada âœ…');
        setTimeout(() => setInlineMsg(avatarMsg, ''), 2000);
      } catch (e) {
