...
function renderItems(docs, { append = false } = {}) {
  ...
  (docs || []).forEach((snap) => {
    ...
    const title = `${ownerName}${level ? ` Â· ${level}` : ''}${topicTitle ? ` Â· ${topicTitle}` : ''}`;
    const body = snippet ? (snippet.length > 160 ? `${snippet.slice(0, 160)}â€¦` : snippet) : '';
    const meta = [
      resolved ? 'Resuelto' : '',
      hasVoice ? 'Voz' : '',
      dateText ? dateText : '',
...
async function loadPage({ reset = false } = {}) {
  ...
  try {
    ...
    if (!docs.length && !lastSnap) {
      if (hintEl) hintEl.textContent = 'AÃºn no hay solicitudes.';
...
onAuthStateChanged(auth, async (user) => {
  ...
  if (!currentUid) {
    if (hintEl) hintEl.textContent = 'Inicia sesiÃ³n para ver la comunidad.';
    if (btnMore) btnMore.hidden = true;
